...
> Срез проекта на 2026-06-15 (обновлено до v0.34.0 — глобальный редизайн лендинга: смена позиционирования на «Обучающая платформа для вайб-кодинга 1С-разработчиков», удаление лишних блоков (фичи, CTA), чистка хедера, удаление чата с лендинга, строгий минималистичный стиль). Для нового разработчика или AI — понять проект за 3-5 минут без чтения всего кода.
>
...
### QuizBlock — полная переделка — ✅ ИСПРАВЛЕНО v0.27.0

> **3 бага одновременно**:
> 1. **Квиз не засчитывал правильные ответы** — AI-промпт генерировал поле `"correct"` (0-based), а QuizBlock ожидал `"correctIndex"`. Несовпадение имён → все ответы считались неверными.
> 2. **Цвет кнопок `bg-purple-600`** — регресс бага 6 (ядовито-фиолетовый). Должен быть `bg-violet-400/50`.
> 3. **Нет таймера** — квиз показывал все вопросы сразу без ограничения по времени, а должен показывать по одному вопросу с таймером 30 секунд.
>
> **Исправления**:
> (1) Нормализация полей: при парсинге quiz JSON проверяем оба варианта — `correctIndex` и `correct`, маппим в `correctIndex`.
> (2) Все purple → violet: иконка `text-violet-300`, кнопки `bg-violet-400/50 hover:bg-violet-400/70`, прогресс-дот `bg-violet-300/70`.
> (3) Полная переделка QuizBlock:
> - По одному вопросу за раз с анимацией перехода
> - Таймер 30 секунд с визуальным прогресс-баром (зелёный → жёлтый → красный)
> - При истечении времени — автоматический пропуск, XP не начисляется
...
| БАГ-25 Glow свечения | ⚠️ Частично | .glow-emerald, .glow-amber, .glow-purple активны в globals.css, но удалены с лендинга в v0.34.0 |

### v0.34.0 — Глобальный редизайн лендинга (2026-06-15)

**Смена позиционирования**: «AI Тренажёр для 1C разработчиков» → «Обучающая платформа для вайб-кодинга 1С-разработчиков»

**Изменения лендинга** (`src/app/page.tsx`):
- Хедер: удалены кнопки «Начать» и «Демо» в правом верхнем углу (оставлен только логотип)
- Удалены секции: «Ежедневные задачи», «Дерево навыков», «Рейтинг и достижения», «Серия дней» (features grid)
- Удалена секция CTA: «Готов прокачать AI-навыки?»
- Футер: удалён копирайт © 2026
- Hero заголовок: заменён на «Обучающая платформа для вайб-кодинга 1С-разработчиков»
- Hero описание: обновлено
- Кнопка «Начать обучение»: уменьшена на 30%, удалены glow-эффекты, строгий минималистичный стиль
- Удалена кнопка «Попробовать демо» из Hero
- Стиль: минималистичный, без избыточных декораций

**Чат поддержки** (`src/app/layout.tsx` → `src/components/layout/app-layout.tsx`):
- MentorChat перемещён из корневого layout в AppLayout — виджет доступен только внутри дашборда, не на лендинге

**Бэкап**: ветка `backup/pre-v0.34.0` создана и запушена в origin
