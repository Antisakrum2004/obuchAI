...
import { Home, Target, Trophy, Flame, Info, Award, BookOpen } from "lucide-react";
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden border-t border-white/5 bg-[#0a0a0f]/95 backdrop-blur-lg safe-area-bottom">
      <div className="flex items-center justify-around h-16">
        ...
        {/* Version indicator */}
        <span className="absolute bottom-1 right-3 text-[8px] text-muted-foreground/30 font-mono">v{process.env.NEXT_PUBLIC_APP_VERSION || "0.37.0"}</span>
      </div>
    </nav>
  );
}
