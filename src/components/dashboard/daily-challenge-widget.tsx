...
export function DailyChallengeWidget({
  ...
  return (
    ...
    >
      {/* Full-width horizontal layout: spread everything across the width */}
      <div className="flex items-center gap-4 w-full">
        {/* Left: flame icon + title */}
        <div className="flex items-center gap-2 shrink-0">
          <Flame className={cn("h-4 w-4 text-amber-400", !completed && "fire-pulse")} />
          <span className="text-sm font-semibold whitespace-nowrap">Ежедневная задача</span>
        </div>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <span className="text-sm">{categoryEmoji(challenge.category)}</span>
